...
export default function AcademicYearSettingsPage() {
  ...
  const createAcademicYear = useMutation(
    api.schools.academicYears.createAcademicYear,
  );
  const activateAcademicYear = useMutation(
    api.schools.academicYears.activateAcademicYear,
  );
  const closeAcademicYear = useMutation(
    api.schools.academicYears.closeAcademicYear,
  );
  const createTerms = useMutation(api.schools.terms.createTerms);

  const selectedAcademicYearId =
    currentAcademicYear?._id ?? academicYears?.[0]?._id;
  ...
  const closedYears = academicYears.filter(
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard
          ...
          helper="Configured year records in this school"
          icon={CalendarDays}
        />
        <MetricCard
          label="Closed years"
          value={String(closedYears)}
          helper="Historical years that are no longer operational"
          icon={CalendarDays}
        />
        <MetricCard
          label="Current year"
          value={currentAcademicYear?.name ?? "None"}
          helper="The active year for current operations"
          icon={CalendarDays}
        />
        <MetricCard
          label="Current term"
          ...
          helper={
            currentTerm
              ? `${currentTerm.daysRemaining} days remaining`
              : "No active term"
          }
          icon={CalendarDays}
        />
      ...
      {!currentAcademicYear && (
        <FeedbackBanner
          tone="warning"
          title="No academic year is active yet"
          description="Activate one year before operational workflows begin so current term and placement context can be derived correctly."
        />
      )}
      ...
      {message && (
        <FeedbackBanner
          tone="success"
          title="The academic calendar has been updated"
          description={message}
        />
      )}
          ...
          <div className="mb-6 flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-600">
                Activate one year at a time and close historical years without
                deleting data.
              </p>
            </div>
            {currentTerm && (
        ...
        <section className="space-y-6">
          <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <form
              ...
              onSubmit={(event) => void handleCreateAcademicYear(event)}
            >
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <input
                    id="year"
                    type="number"
                    {...yearForm.register("year")}
                    className="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm shadow-sm transition-all focus:border-school-primary focus:outline-none focus:ring-2 focus:ring-school-primary/20"
                  />
                </div>
                <div>
                  <label
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-lg bg-school-primary px-4 py-2.5 text-sm font-medium text-white shadow-sm transition-all duration-200 hover:bg-crimson-dark active:scale-95 disabled:cursor-not-allowed disabled:opacity-70"
              >
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-600">
                  Create the operational terms for the selected academic year.
                </p>
              </div>
            ...
            {canCreateTerms && (
              <form
                className="mt-6 space-y-4"
                onSubmit={(event) => void handleCreateTerms(event)}
              >
                {fieldArray.fields.map((field, index) => (
                  <div
                    key={field.id}
                    className="grid gap-4 rounded-xl border border-gray-200 p-4 md:grid-cols-[1.2fr_1fr_1fr]"
                  >
                    <div>
                      <label
                      ...
                      <input
                        id={`terms.${index}.name`}
                        type="text"
                        {...termForm.register(`terms.${index}.name`)}
                        className="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm shadow-sm transition-all focus:border-school-primary focus:outline-none focus:ring-2 focus:ring-school-primary/20"
                      />
                    </div>
                    <div>
                      <label
                        htmlFor={`terms.${index}.startDate`}
                        className="mb-1 block text-sm font-medium text-gray-700"
                      >
                        Start date
                      </label>
                      <input
                        ...
                        type="date"
                        {...termForm.register(`terms.${index}.startDate`)}
                        className="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm shadow-sm transition-all focus:border-school-primary focus:outline-none focus:ring-2 focus:ring-school-primary/20"
                      />
                    </div>
                    <div>
                      <label
                        htmlFor={`terms.${index}.endDate`}
                        className="mb-1 block text-sm font-medium text-gray-700"
                      >
                        End date
                      </label>
                      <input
                        ...
                        type="date"
                        {...termForm.register(`terms.${index}.endDate`)}
                        className="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm shadow-sm transition-all focus:border-school-primary focus:outline-none focus:ring-2 focus:ring-school-primary/20"
                      />
                    </div>
                  </div>
                ...
                <button
                  type="submit"
                  disabled={termForm.formState.isSubmitting}
                  className="inline-flex items-center gap-2 rounded-lg bg-school-primary px-4 py-2.5 text-sm font-medium text-white shadow-sm transition-all duration-200 hover:bg-crimson-dark active:scale-95 disabled:cursor-not-allowed disabled:opacity-70"
                >
                  {termForm.formState.isSubmitting ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
                  ) : (
                    <Plus className="h-4 w-4" />
                  )}
